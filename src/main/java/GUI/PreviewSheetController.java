		...
		for (int i = 0; i < numOfMeasures; i++) { // go through every measure
			...
			for (int j = 0; j < measures.get(i).getNumNotes(); j++) { // go through all notes in specific measure
				...
				case "P1-I48":
					...
					break;
				case "P1-I45":
					instrumentID = "[PEDAL_HI_HAT]";
					break;
				case "P1-I47":
					instrumentID = "[OPEN_HI_HAT]";
					break;
				case "P1-I50":
				...
				case "P1-I39":
					...
					break; // or [ELECTRIC_SNARE]
				case "P1-I54":
					instrumentID = "[RIDE_BELL]";
					break;
				case "P1-I53":
					instrumentID = "[CHINESE_CYMBAL]";
					break;
				case "P1-I36":
					instrumentID = "[BASS_DRUM]";
					break;
				case "P1-I52":
					instrumentID = "[RIDE_CYMBAL_1]";
					break;
				}
			...
			for (int j = 0; j < noteList.size(); j++, x += noteSpacing) {
				...
				if (p.getInstrument().equals("drumset")) {
					...
					if (note.getUnpitched().getOctave() == 5) {
						...
						case 'A':
							y = 1;
							Line strikethrough = new Line(x - 2, -12 + (y - 1) * 6 + yStaff, x + 12,
									-12 + (y - 1) * 6 + yStaff);
							pane.getChildren().add(strikethrough);
							break;
						case 'G':
							y = 2;
							break;
